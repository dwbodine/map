import type { Photo } from 'react-photo-album';

const breakpoints = [960, 640, 384, 256];

function imageLink(
  path: string,
  width: number,
  height: number,
  size: number,
  extension: string
) {
  return `/galleries/justforfun/${path}.${width}x${height}.${size}w.${extension}`;
}

const justforFunPhotos: Photo[] = [
  {
    src: 'IMG_0536.960x640.jpg',
  },
  {
    src: 'IMG_0580.960x640.jpg',
  },
  {
    src: 'IMG_1017.960x640.jpg',
  },
  {
    src: 'IMG_3307.960x640.jpg',
  },
  {
    src: 'IMG_4157.960x640.jpg',
  },
  {
    src: 'IMG_5796-2.960x640.jpg',
  },
].map(({ src, ...rest }) => {
  const matcher = src.match(/^(.*)\.(\d+)x(\d+)\.(.*)$/)!;

  const path = matcher[1] || '';
  const width = Number.parseInt(matcher[2] || '0', 10);
  const height = Number.parseInt(matcher[3] || '0', 10);
  const extension = matcher[4] || '';

  return {
    key: path,
    src: imageLink(path, width, height, width, extension),
    width,
    height,
    srcSet: breakpoints.map((breakpoint) => ({
      src: imageLink(path, width, height, breakpoint, extension),
      width: breakpoint,
      height: Math.round((height / width) * breakpoint),
    })),
    ...rest,
  } as Photo;
});

export default justforFunPhotos;
